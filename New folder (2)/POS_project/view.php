<?php 
    include("includes/header.php");
    include("includes/sidebar.php");
    include("includes/nav.php");
?>

<?php 
$conn = mysqli_connect('localhost','root','','pos_project1');
if (isset($_GET['deleteid'])){ 
    $deleteid = $_GET['deleteid'];

     $sql = "DELETE FROM  product WHERE id = $deleteid";
     if(mysqli_query($conn, $sql) == TRUE){ 
        header('location:view.php');
     }
}


?>

<head>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">

</head>
   <div class="container"> 
    <div class="row"> 
    <p>
     
    
         

    <span class='btn btn-info'><a href='insert_product.php' class='text-white text-decoration-none'>Add New Product</a></span>
    
       
    </p>
        <div class="col-sm-1"></div>
        <div class="col-sm-10 pt-4 mt-4 border border-success bg-dark text-white">
        
            <h3 class="text-center p-2 m-2 bg-secondary text-white">Product Information</h3>
           
            <?php 
            $sql = 'SELECT product.id,product.pname,category.catname,sub_category.subcatname,product.cat_id,product.sub_category_id,suppliers.supplier_id,product.price,product.manufacturer_id FROM category,sub_category,suppliers,product where product.cat_id = category.id and product.sub_category_id = sub_category.id and product.supplier_id= suppliers.id';
            
            $query = mysqli_query($conn, $sql);
            echo "<table class='table table-success'>
             <tr class='table-dark'>
                <th>ID</th>
                <th>PRODUCT NAME</th>
                <th>CATEGORY NAME</th>
                <th>SUBCATEGORY NAME</th>
                <th>SUPPLIERS NAME</th>
                <th>PRICE</th>
                <th>MANUFACTURER_ID</th>
                <th>ACTION</th>
             </tr>";
           while ($data = mysqli_fetch_assoc($query)){ 

            $id = $data['id'];
            $name = $data['pname'];
            $p_id = $data['catname'];
            $subcatname = $data['subcatname'];
            $suppliername = $data['supplier_id'];
            $price = $data['price'];
            $manufac = $data['manufacturer_id'];
            echo "<tr> 
                    <td>$id</td>
                    <td>$name</td>
                    <td>$p_id</td>
                    <td>$subcatname</td>
                    <td>$suppliername</td>
                    <td>$price</td>
                    <td>$manufac</td>
                    <td>
                    <span class='btn btn-info'><a href='edit.php?id=$id' class='text-white text-decoration-none'>Edit</a></span>
                    <span class='btn btn-danger'><a href='view.php?deleteid=$id' class='text-white text-decoration-none'>Delete</a></span>
                    </td>
                </tr>";
           }
            ?>
        </div>
        <div class="col-sm-1"></div>
       
  
    </div>
   </div>
  
 
 
  
   <?php 
    include("includes/footer.php");
?>
